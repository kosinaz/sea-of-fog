...
func _draw():
	...
	for tile in explored:
		...
		if [8, 9, 18, 19, 65, 67].has($"%TileMap".get_cellv(tile_vector)):
			color = Color.gray
		if [49, 50, 74].has($"%TileMap".get_cellv(tile_vector)):
			color = Color.white
		if [11, 53, 56].has($"%TileMap".get_cellv(tile_vector)):
			color = Color.black
		if [4, 10, 12, 17, 20, 22, 23, 24, 25, 28, 29, 30, 31, 37, 54, 62, 63].has($"%TileMap".get_cellv(tile_vector)):
